...
write_dockerfile_maven()
{
  ...
  then
    ...
    write_dockerfile_body "${OUTPUT_DIR}/DockerSubcomponent_maven"
    prepare_docker_contents
    RC=$?
  else
    DOCKER_SUBIMAGE_MAPPING+=" maven:${CURRENT_IMAGE_ID}"
    ...
    __record_addon_variables "${outputfile}" "${ENV_SETTINGS_MAVEN}"
